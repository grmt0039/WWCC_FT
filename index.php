<?php include("includes/head.php"); ?>
  <?php include("includes/header.php"); ?>

    <div class="content-wrapper">
      <div id="hero">
          <!--<div><img src="images/hero/hero.jpg" alt=""></div>-->
        <blockquote>
          <a href="#">&#34;The workshops helped me to find a job and secure day-care for my son.&#34;  -  Sanjana</a>
        </blockquote>
      </div>
       
      <div id="steps-wrapper">
        <div class="container">  
          <ul id="steps">
            <li>
              <a href="#about">
                <?php echo '<?xml version="1.0" encoding="utf-8"?>
<!-- Generator: Adobe Illustrator 17.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
   width="134px" height="134px" viewBox="0 0 170 155" enable-background="new 0 0 170 155" xml:space="preserve">
<g>
  <polygon class="heart" fill="#B61E54" points="110.198,108.753 115.357,103.654 110.198,103.654   "/>
  <path class="heart" fill="#B61E54" d="M103.302,101.772c0-2.832,2.304-5.136,5.136-5.136h10.497V24.122H56.839v88.144h46.464v-10.494H103.302z
     M87.775,36.272c4.175,0,7.56,3.384,7.56,7.559s-3.385,7.559-7.56,7.559s-7.559-3.384-7.559-7.559S83.6,36.272,87.775,36.272z
     M72.81,86.46h8.721V60.314H72.81v-4.376h21.735V86.46h8.788v4.36H72.81V86.46z"/>
  <path class="heart" fill="#B61E54" d="M155.534,14.351C136.229-4.853,105.013-4.777,85.81,14.538C66.606-4.777,35.392-4.853,16.087,14.351
    C-3.219,33.548-3.667,64.505,17.562,85.737c23.7,23.688,61.614,61.622,68.244,68.263c6.641-6.641,44.548-44.573,68.248-68.263
    C175.293,64.505,174.839,33.546,155.534,14.351z M107.512,115.184V103.3c0-1.286,1.047-2.333,2.334-2.333h12.05L107.512,115.184z
     M121.969,99.67h-13.53c-1.158,0-2.102,0.943-2.102,2.102V115.3H53.805v-0.797V21.087h68.164
    C121.969,21.087,121.969,99.67,121.969,99.67z"/>
</g>
</svg>' ?>
                <h2>What we do</h2>
              </a>
            </li>
            <li>
              <a href="#volunteer">
                <?php echo '<?xml version="1.0" encoding="utf-8"?>
<!-- Generator: Adobe Illustrator 17.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
   width="134px" height="134px" viewBox="-35 -35 170 170" enable-background="new -35 -35 170 170" xml:space="preserve">
<g>
  <path class="heart" fill="#B61E54" d="M18.326,37.109v31.054c0,17.082,13.817,30.978,30.802,30.978c16.974,0,30.797-13.778,30.916-30.759
    c-0.01-0.072-0.016-0.145-0.016-0.219V21.373c-0.006-2.977-2.44-4.059-4.075-4.059c-1.052,0.001-2.084,0.4-2.837,1.093
    c-0.82,0.757-1.236,1.762-1.234,2.985v27.92c0,0.015,0,0.029-0.001,0.044c0.001,0.013,0.001,0.025,0.001,0.038
    c0,1.514-1.232,2.746-2.745,2.746c-1.514,0-2.746-1.232-2.746-2.746c0-0.012,0-0.025,0.001-0.038
    c-0.001-0.014-0.001-0.029-0.001-0.043V6.706c-0.007-2.977-2.441-4.059-4.075-4.059c-1.053,0.001-2.085,0.4-2.837,1.093
    c-0.822,0.757-1.237,1.761-1.235,2.985v42.667c0,1.514-1.232,2.746-2.745,2.746c-1.514,0-2.746-1.232-2.746-2.746V2.199
    c-0.007-2.978-2.44-4.06-4.075-4.06c-1.051,0.002-2.083,0.401-2.836,1.094c-0.822,0.758-1.237,1.762-1.235,2.985v47.175
    c0,1.514-1.231,2.746-2.745,2.746c-1.514,0-2.746-1.232-2.746-2.746V10.8c-0.006-2.978-2.44-4.061-4.075-4.061
    c-1.05,0.001-2.081,0.401-2.835,1.095c-0.822,0.756-1.237,1.76-1.235,2.985v48.692c5.861,0.447,11.532,2.333,16.486,5.492
    c0.085,0.054,0.165,0.117,0.237,0.187c0.665,0.52,1.054,1.297,1.054,2.135c0,1.514-1.232,2.746-2.746,2.746
    c-0.268,0-0.533-0.04-0.793-0.119c-0.19-0.035-0.373-0.105-0.538-0.212c-4.894-3.167-10.581-4.841-16.447-4.841h-1.23
    c-0.838,0-1.516-0.679-1.516-1.516V43.283c-0.008-4.009-3.197-6.175-6.194-6.175l-0.013-1.516l0.002,1.516L18.326,37.109z"/>
  <path class="heart" fill="#B61E54" d="M120.428-12.21c-19.446-19.344-50.89-19.268-70.234,0.188C30.849-31.478-0.593-31.555-20.039-12.21
    c-19.448,19.337-19.899,50.52,1.486,71.908C5.319,83.559,43.511,121.771,50.19,128.46c6.689-6.689,44.874-44.9,68.747-68.762
    C140.332,38.31,139.874,7.125,120.428-12.21z M49.126,104.635c-19.901,0-36.159-16.243-36.288-36.227
    c-0.002-0.035-0.003-0.069-0.003-0.104V34.371c0-1.514,1.232-2.746,2.746-2.746l3.683-0.007c2.191,0,4.36,0.633,6.214,1.765V10.825
    c-0.007-2.74,1.061-5.238,3.006-7.031c1.757-1.615,4.14-2.541,6.54-2.545c1.414,0,2.817,0.319,4.092,0.911
    c0.011-2.717,1.077-5.19,3.007-6.965c1.755-1.615,4.138-2.543,6.539-2.546c3.486,0,7.28,1.977,8.844,5.783
    c1.448-0.82,3.116-1.273,4.795-1.275c4.708,0,9.57,3.571,9.583,9.546v6.016c1.262-0.579,2.655-0.894,4.056-0.896
    c4.706,0,9.568,3.571,9.58,9.545v46.596c0.009,0.067,0.014,0.134,0.014,0.203C85.532,88.276,69.201,104.635,49.126,104.635z"/>
</g>
</svg>
' ?>
                <h2>Volunteer</h2>
              </a>
            </li>
            <li>
              <a href="#donate">
                <?php echo '<?xml version="1.0" encoding="utf-8"?>
<!-- Generator: Adobe Illustrator 17.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
   width="134px" height="134px" viewBox="0 0 170 155" enable-background="new 0 0 170 155" xml:space="preserve">
<path class="heart" fill="#B61E54" d="M100.038,84.764c-1.777-1.073-4.316-2.062-7.483-2.968v21.337c1.209-0.202,2.331-0.474,3.322-0.817
  c5.43-1.904,8.143-5.421,8.143-10.572C104.021,88.742,102.699,86.416,100.038,84.764"/>
<path class="heart" fill="#B61E54" d="M68.076,56.881c0,3.124,1.416,5.46,4.242,6.992c1.205,0.678,3.3,1.427,6.263,2.259V46.61
  c-2.392,0.467-4.497,1.214-6.197,2.352C69.509,50.844,68.076,53.502,68.076,56.881"/>
<path class="heart" fill="#B61E54" d="M154.478,15.071C135.241-4.065,104.135-3.99,84.999,15.256C65.862-3.99,34.758-4.065,15.521,15.071
  C-3.718,34.201-4.164,65.048,16.99,86.207c23.617,23.605,61.398,61.406,68.005,68.023c6.618-6.617,44.392-44.417,68.009-68.023
  C174.169,65.048,173.716,34.199,154.478,15.071 M110.157,109.169c-4.539,3.889-10.571,6.217-18.06,7.034v8.205H78.109v-8.301
  c-7.176-0.904-13.076-3.181-17.685-6.814c-6.151-4.856-9.221-11.515-9.221-19.997h16.258c0.521,3.728,1.564,6.512,3.104,8.358
  c1.742,2.062,4.261,3.48,7.541,4.295V79.774c-8.373-1.879-14.158-3.933-17.341-6.128c-5.492-3.758-8.244-9.646-8.244-17.643
  c0-7.307,2.688-13.357,8.074-18.196c4.247-3.826,10.101-6.11,17.513-6.905v-7.287c0-0.013-0.005-0.013-0.005-0.013l13.995,0.025
  v7.636c5.408,0.993,10.223,3.013,14.437,6.127c6.146,4.568,9.355,11.163,9.663,19.828H99.823c-0.31-4.894-2.495-8.381-6.584-10.445
  c-0.356-0.184-0.752-0.315-1.144-0.47v20.516l5.296,1.247c6.27,1.473,10.972,3.451,14.104,5.908
  c4.858,3.845,7.295,9.397,7.295,16.659C118.796,98.07,115.905,104.27,110.157,109.169"/>
<rect x="1.105" y="0.77" fill="none" width="167.789" height="153.46"/>
</svg>
' ?>
                <h2>Donate</h2>
              </a>
            </li>
          </ul>
        </div>
      </div>
    

      <div id="mission-wrapper">
        <div class="container">
          <p>We provide immigrant women and their families with opportunities to improve the quality of their lives through self-development and community action.</p>
        </div>
      </div>


      <section id="events-wrapper">
        <div class="container">
          <div id="calendar-wrapper">
            <a href="#calendar"><?php echo '<?xml version="1.0" encoding="utf-8"?>
<!-- Generator: Adobe Illustrator 17.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
   width="72px" height="65.016px" viewBox="0 0 72 65.016" enable-background="new 0 0 72 65.016" xml:space="preserve">
<g id="Calendar">
  <path class="calendar" fill="#B51E52" d="M65.531,6.57h-3.392V5.652c0-2.749-2.461-4.999-5.469-4.999h-0.081c-3.008,0-5.469,2.25-5.469,4.999V6.57
    H20.876V5.652c0-2.749-2.461-4.999-5.469-4.999h-0.081c-3.008,0-5.469,2.25-5.469,4.999V6.57H6.469C3.461,6.57,1,8.82,1,11.569
    v47.794c0,2.749,2.461,4.999,5.469,4.999h59.062c3.008,0,5.469-2.25,5.469-4.999V11.569C71,8.82,68.539,6.57,65.531,6.57z
     M53.308,5.652c0-1.652,1.47-2.999,3.281-2.999h0.081c1.807,0,3.281,1.347,3.281,2.999v8.893c0,1.652-1.474,2.999-3.281,2.999
    h-0.081c-1.811,0-3.281-1.347-3.281-2.999V5.652z M12.044,5.652c0-1.652,1.47-2.999,3.281-2.999h0.081
    c1.807,0,3.281,1.347,3.281,2.999v8.893c0,1.652-1.474,2.999-3.281,2.999h-0.081c-1.812,0-3.281-1.347-3.281-2.999V5.652z
     M65.531,60.363H6.469c-0.594,0-1.094-0.461-1.094-1v-35.14h61.25v35.141C66.625,59.903,66.125,60.363,65.531,60.363z"/>
</g>
<text transform="matrix(1 0 0 1 21.4438 40.008)"><tspan x="0" y="0" font-family="Helvetica" font-size="12">View</tspan><tspan x="-10.896" y="15.2" font-family="Helvetica" font-size="12">Calendar</tspan></text>
</svg>' ?></a>
          </div>
          
          <article class="event">
            <img src="images/calendar/calendar1.jpg">
            <p class="date">Nov 22</p>
            <h3>Victoria Fun Crafts</h3>
            <p class="blurb">Join fellow craft enthusiasts from community groups supported by WWCC as local craft stars host.</p>
            <p class="link"><a src="#event1">Learn More</a></p>
          </article>
          <article class="event">
            <img src="images/calendar/calendar2.jpg">
            <p class="date">Oct 09</p>
            <h3>Youth Celebration</h3>
            <p class="blurb">Come enjoy the celebration of community projects by 20 youth in GTA next weekend.  Prizes available.</p>
            <p class="link"><a src="#event2">Learn More</a></p>
          </article>
          <article class="event">
            <img src="images/calendar/calendar3.jpg">
            <p class="date">Sep 15</p>
            <h3>Mind-Body Workshop</h3>
            <p class="blurb">Connect with your expressive self and your imagination. Explore your body and mind during this retreat. </p>
            <p class="link"><a src="#event3">Learn More</a></p>
          </article>

        </div>
      </section> 

    </div>

  <?php include("includes/footer.php"); ?>
  